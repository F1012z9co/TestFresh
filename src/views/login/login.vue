        ...
        <div class="fr form-box">
          ...
          <form id="jsLoginForm" autocomplete="off">
            ...
            <div class="form-group marb20">
              <label>用&nbsp;户&nbsp;名</label>
              <input name="account_l" id="account_l" type="text" placeholder="手机号/账号" v-model="userName" @focus="errorUnshow">
            </div>
            <p class="error-text" v-show="userNameError">{{userNameError}}</p>
            <div class="form-group marb8">
              <label>密&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;码</label>
              <input name="password_l" id="password_l" type="password" placeholder="请输入您的密码" v-model="parseWord" @focus="errorUnshow">
            </div>
             <p class="error-text" v-show="parseWordError">{{parseWordError}}</p>
            <div class="auto-box marb38">
            </div>
            <p class="error-text" v-show="error">{{error}}</p>
            <input class="btn btn-green" id="jsLoginBtn" type="button" value="立即登录 &gt; " @click="login">
          </form>
...
export default {
  data () {
    return {
      ...
      parseWord: '',
      error: '', // 公用的错误信息
      userNameError: '', // 用户名的错误信息
      parseWordError: '' // 密码的错误信息
    }
  },
  methods: {
    login () {
