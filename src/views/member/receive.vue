            ...
            <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd" v-for="(item, index) in receiveInfoArr" :key="index">
              <tbody>
                <tr>
                  ...
                  <td colspan="3" align="left" bgcolor="#ffffff">
                    <div class="addr" @click="bubble(index)">
                      <!-- 三级级联菜单 -->
                      <v-distpicker :province="item.province" :city="item.city" :area="item.district" @selected="onSelected"></v-distpicker>
                    </div>
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td align="left" bgcolor="#ffffff"><input name="mobile" type="text" class="inputBg" v-model="item.signer_mobile"><span :class = "{error:item.signer_mobile==''}">(必填)</span></td>
                </tr>
                <tr>
              ...
              <tbody>
                <tr>
                  ...
                  <td colspan="3" align="left" bgcolor="#ffffff">
                    <div class="addr">
                      <!-- 三级级联菜单 -->
                      <v-distpicker :province="newInfo.province" :city="newInfo.city" :area="newInfo.district" @selected="getSelected"></v-distpicker>
                ...
                <tr>
                  <td align="right" bgcolor="#ffffff">手机：</td>
                  <td align="left" bgcolor="#ffffff"><input name="mobile" type="text" class="inputBg" v-model="newInfo.signer_mobile"><span :class = "{error:newInfo.signer_mobile==''}">(必填)</span></td>
                </tr>
                <tr>
                  <td align="right" bgcolor="#ffffff">&nbsp;</td>
                  <td colspan="3" align="center" bgcolor="#ffffff">
  ...
  methods: {
    ...
    addReceive () { // 提交收货信息
      addAddress(this.newInfo).then((response) => {
        ...
        // 重置新的
        this.getReceiveInfo()
        console.log('置空前' + this.newInfo.address)
        Object.assign(this.newInfo, this.newInfoEmpty)
        console.log('置空后' + this.newInfo.address)
      }).catch(function (error) {
        console.log(error)
      })
    }
