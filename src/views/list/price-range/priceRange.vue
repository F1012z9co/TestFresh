      ...
      <dl>
        ...
        <dd class="dd-price" style="height: 48px;">
          <div class="items cle w500">
            <!-- 区间列表 -->
            <div class="link" v-for="(item,index) in priceRange" :key="index" @click="changePriceRange(item.min,item.max)">
          ...
          <div class="priceform" id="priceform" v-clickoutside="handleClickOutside">
            <div class="form-bg">
              <form id="freepriceform" >
                ...
                <input v-show="focus" value="确定" class="submit" @click="changePriceRange(price_min, price_max)">
              </form>
            </div>
          </div>
        </dd>
...
a.more-btn,.rmb,.search-selected a.item,.sort .bd a span{background:url(../../../../static/images/search-page-bg.png) 0 -9999px no-repeat}
