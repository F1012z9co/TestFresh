      ...
      <div class="bd_bar_bd cle">
        <ul class="welcome">
          <li v-if="userInfo.name">
            <a href="userinfo.html">{{userInfo.name}}</a>
            &nbsp;[
    ...
    <div class="hd_main cle">
      <div class="logo">
        <router-link to="/app/home/index" class="lizi_logo">
            <img src="../../../static/images/head/logo.jpg" alt="生鲜商城">
        </router-link>
      </div>
      ...
      <div class="head_search_hot">
        <span>热搜榜：</span>
        <router-link :to="'/app/home/search/'+item.keywords" v-for="item in hotSearch" :key="item.keywords">{{item.keywords}}</router-link>
      </div>
      <div class="intro">
        <ul>
          <li class="no1">
    ...
    <div class="hd_nav">
      <div class="hd_nav_bd cle">
            ...
            <div class="main_cata" id="J_mainCata" v-show="showAllmenu">
              <ul>
                <li class="first" v-for="(item,index) in allMenuLabel" :key="item.id" @mouseover="overChildrenmenu(index)" @mouseout="outChildrenmenu()">
                  ...
                  <div class="J_subCata" v-show="showChildrenMenu===index" style="left: 215px; top: 0px;">
                    <div class="J_subView" >
                      <div v-for="list in item.sub_cat" :key="list.id">
                        <dl>
                          ...
                          <dd>
                            <router-link :to="'/app/home/list/'+childrenList.id" v-for="childrenList in list.sub_cat" :key="childrenList.id">{{childrenList.name}}</router-link>
                          </dd>
                        </dl>
                        <div class="clear"></div>
                      </div>
        ...
        <ul class="sub_nav cle" id="sub_nav">
            ...
            <li v-for="(item,index) in allMenuLabel" :key="index">
        ...
        <div class="hd_cart" id="ECS_CARTINFO" @mouseover="overShopCar" @mouseout="outShopCar">
          <a class="tit" href="cart.html" target="_blank">
            ...
            <em class="num" id="hd_cartnum" style="visibility: visible;">{{goodsList.goods_list.length}}</em>
          </a>
          <!-- display控制购物车显示隐藏 -->
          <div class="list" v-show="showShopCar">
            <div class="data">
              <dl v-for="(item,index) in goodsList.goods_list" :key="index">
            ...
            <div class="count">共<span class="red" id="hd_cart_count">{{goodsList.goods_list.length}}</span>件商品哦~
              <p>
                总价:<span class="red"><em id="hd_cart_total">{{goodsList.totalPrice}}</em></span>
                <router-link class="btn" :to="'/app/shoppingcart/cart'" target="_blank">去结算</router-link>
              </p>
            </div>
          </div>
        </div>
...
export default {
  data () {
    return {
      hotSearch: [], // 热词
      searchWord: '', // 搜索词
      showAllmenu: false, // 全部商品分类显示控制
      allMenuLabel: [], // 各级菜单列表
      showChildrenMenu: -1, // 二、三级子菜单显示控制
      showShopCar: false, // 购物车显示控制
      isShowVip: false // 会员中心子菜单显示控制
    }
  },
  computed: {
  ...
  methods: {
    ...
    deleteGoods (index, id) { // 移除购物车
      deleteShopCart(id).then((response) => {
        ...
        this.goodsList.goods_list.splice(index, 1)
...
.hd_bar ul#userinfo-bar li .vipico {
    width:12px;
    height:17px;
   /*background:url(../images/head/wap.png) no-repeat;*/
    float:left;
    margin-right:4px;
    font-size:18px
}
