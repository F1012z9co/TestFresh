        ...
        <div class="detail_img" id="detail_img">
          <div class="pic_view">
            <a class="MagicZoomPlus" id="Zoomer" style="position: relative; display: inline-block; text-decoration: none; outline: 0px; overflow: hidden; width: auto; height: auto;">
              <!-- 大图展示 -->
              <img :src="curShow.image" :alt="curShow.desc" style="opacity: 1;">
            ...
            <div class="bd">
              <div class="tempWrap" style="overflow:hidden; position:relative; width:330px">
                <ul class="cle" style="width: 330px; position: relative; overflow: hidden; padding: 0px; margin: 0px; left: 0px;">
                  <!-- 小图列表项 -->
                  <li v-for="(item,index) in proDetail.images" :key="index" :class="{'current': index===curIndex}" @click="replaceShow(index,item)">
                    <a>
        ...
        <div class="item-info" id="item-info">
          <dl class="loaded">
            ...
            <dd class="property">
              <ul>
                <!-- 根据后台数据设置是否显示 -->
                <li v-if="proDetail.ship_free">
                  此商品为免运费商品，计算配送金额时将不计入配送费用
                </li>
                <li>
                  <span class="lbl">市场价</span> <em class="cancel">￥{{proDetail.market_price}}元</em>
                </li>
                <li>
              ...
              <ul class="sku">
                <li class="skunum_li cle">
                  ...
                  <div class="skunum"> <span class="minus" title="减少1个数量" @click="reduceNum"><i class="iconfont">-</i></span>
                    <input id="number" name="number" type="text" min="1" v-model="buyNum" readonly>
...
import { getGoodsDetail, getFav, addFav, delFav, addShopCart } from '../../api/api'
...
export default {
  ...
  created () {
    ...
    if (cookie.getCookie('token')) {
      getFav(this.productId).then((response) => {
        // console.log(response.data)
        this.hasFav = response.data.hasFav
  ...
  methods: {
    getDetails () { //  请求商品详情
      getGoodsDetail(this.productId).then((response) => {
        // console.log(response.data)
...
.detail {
    ...
    /*background:#fff url(../imagesseemore-bg.png) right 0 repeat-y;*/
    /*background:#fff url(../imagesseemore-bg.png) right 0 repeat-y;*/
}
.detail_wrap {
    width: 1000px;
...
#item-thumbs .prev {
    ...
    width:15px;
    height:30px;
    background:url(../../../static/images/footprint-arr.png) no-repeat;
    top:30px;
    left:18px
}
.item-thumbs .next {
    ...
    width:15px;
    height:30px;
    background:url(../../../static/images/footprint-arr.png) -15px 0 no-repeat;
    top:30px;
    right:18px
}
...
.item-info dl {
    height:425px;
    padding-bottom:10px;
    background:url(../../../static/images/productDetail/images/loading_nala.gif) 250px 200px no-repeat
}
.item-info dl.loaded {
    height:auto;
    background:0
