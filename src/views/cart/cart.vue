<template>
  <div id="main">
    <div class="top-next cle">
      <div class="fl">
        <a class="graybtn" @click="continueShopping">继续购物</a>
        <a class="btn" id="checkout-top" @click="balanceCount">&nbsp;去结算&nbsp;</a>
      </div>
    </div>
    <div class="cart-box" id="cart-box">
      <div class="hd">
        <span class="no2" id="itemsnum-top">{{goods_list.length}}件商品</span>
      ...
      <div class="goods-list">
        <ul>
          <!-- 购物车项 -->
          <li class="cle hover" style="border-bottom-style: none;" v-for="(item,index) in goods_list" :key="index">
            <div class="pic">
            ...
            <div class="price-xj"><span></span>
              <em>￥{{item.goods.shop_price * item.nums}}元</em>
            </div>
            <div class="del">
      ...
      <div class="fd cle">
        ...
        <div class="fr" id="price-total">
          <p><span id="selectedCount">{{goods_list.length}}</span>件商品，总价：<span class="red"><strong id="totalSkuPrice">￥{{totalPrice}}元</strong></span></p>
        </div>
        <div class="extr">
          <div class="address">
            ...
            <ul>
              <li class="add">
                <router-link :to="'/app/home/member/receive'" target = _blank> + 点击添加地址</router-link>
              </li>
              <!-- 收货人地址 -->
              <li v-for="item in addrInfo" :key="item.id" :class="{'addressActive':addressActive==item.id}" @click="selectAddr(item)">
          ...
          <div class="pay">
            ...
            <p class="payWrap">
              <!-- 支付方式 -->
              <img v-for="item in payWrapList" :key="item.id" :src="item.img" :class="{'payWrapActive':payWrapActive==item.id}" @click="selectPay(item.id)">
            </p>
          </div>
        </div>
        <textarea v-model="post_script" placeholder="请输入留言" style="margin-top: 10px; height:50px;width: 100%;"></textarea>
...
<script>
import {getShopCarts, deleteShopCart, updateShopCart, delAllShopCart, getAddress, createOrder} from '../../api/api'
...
export default {
  ...
  methods: {
    ...
    reduceCartNum (index, id) { // 删除数量
      if (this.goods_list[index].nums <= 1) {
        this.deleteGoods(index, id)
      } else {
        updateShopCart(id, {
          nums: this.goods_list[index].nums - 1
        }).then((response) => {
          // console.log(response.data)
          this.goods_list[index].nums = this.goods_list[index].nums - 1
          // 更新store数据
          this.$store.dispatch('setShopList')
          // 更新总价
          this.setTotalPrice()
    ...
    setTotalPrice () {
      ...
      for (var i = 0; i < goodsList.length; i++) {
        totalPrice = totalPrice + goodsList[i].nums * goodsList[i].goods.shop_price
      }
      this.totalPrice = totalPrice
    },
    delAll () { // 清空购物车
      delAllShopCart().then((response) => {
        // console.log(response.data)
    ...
    selectAddr (addrItem) { // 选择配送地址
      this.addressActive = addrItem.id
      this.address = addrItem.province + addrItem.city + addrItem.district + addrItem.address
      this.signer_mobile = addrItem.signer_mobile
      this.signer_name = addrItem.signer_name
      console.log('选择配送地址:' + this.address + this.signer_mobile + this.signer_name)
    },
    selectPay (id) {
      this.payWrapActive = id
      console.log('支付方式：' + this.payWrapActive)
    },
    balanceCount () { // 结算
      if (this.address === '') {
        alert('请选择收货地址')
